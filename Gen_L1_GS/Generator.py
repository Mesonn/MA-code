...
class Block(nn.Module):
    def __init__(self,in_channels, out_channels, down = True, act = "relu", use_dropout = False):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, 4,2,1, bias = False,padding_mode= "replicate")
            if down           
...
class Generator(nn.Module):
    def __init__(self, in_channels = 3, features = 64):
        ...
        nn.Conv2d(in_channels,features,4,2,1,padding_mode="replicate"),
        nn.LeakyReLU(0.2),
        )
        ...
        self.bottelneck  = nn.Sequential(
            ...
            nn.Conv2d(features*8, features*8,4,2, padding =0),
            nn.ReLU(),
            
        )
        self.first_up = nn.Sequential(
            nn.ConvTranspose2d(features, features*8,4,2,padding=0 ),
            nn.ReLU(),
            nn.Dropout(0.5)
        )
